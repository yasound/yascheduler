...
#         self.manager.handle_playlist_history_event(TransientRadioHistoryManager.TYPE_PLAYLIST_ADDED, r.uuid, p.id)
#         self.assertEqual(self.manager.builder.playlist_events.count(), 1)
#         self.manager.builder.handle_playlist_events()
#         self.assertEqual(self.manager.builder.playlist_count(), 1)
#         self.assertEqual(self.manager.builder.playlist_events.count(), 0)
...
class TestJingleManager(TestCase):

    def setUp(self):
        self.jingle_db = settings.MONGO_DB.jingles
        self.jingle_db.drop()

        self.manager = JingleManager()
        self.manager.reset()

    def test_import_jingle(self):
        radio_uuid1 = 'uuid1'
        name1 = 'name1'
        filename1 = 'filename1'
        duration1 = 3
        ...
        self.jingle_db.insert(j1)
        j1_id = self.jingle_db.find_one({'name': name1})['_id']

        self.manager.jingle_added(j1_id)

        self.assertEqual(1, self.manager.count())
        self.assertEqual(1, self.manager.count(radio_uuid1))
        self.assertEqual(0, self.manager.count('unknown uuid'))

    def test_chose_jingle(self):
        radio_uuid1 = 'uuid1'
        name1 = 'name1'
        filename1 = 'filename1'
        duration1 = 3
        ...
        jingle = self.manager.get_jingle(radio_uuid1)
        self.assertIsNotNone(jingle)

        for i in range(song_count1 - 1):
            jingle = self.manager.get_jingle(radio_uuid1)
            self.assertIsNone(jingle)
            current_songs_to_wait = self.manager.db.find_one({'name': name1})['current_songs_to_wait']
            self.assertEqual(current_songs_to_wait, song_count1 - (i + 1))

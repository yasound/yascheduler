...
class RadioScheduler():
    ...
    def __init__(self, enable_ping_streamers=True, enable_programming_check=False, enable_time_profiling=False, radio_offset=None, radio_limit=None):
        ...
        self.history_manager = TransientRadioHistoryManager(self.radio_state_manager, [self.handle_radio_history_event], [self.playlist_manager.handle_playlist_history_event], [self.handle_jingle_history_event])
    ...
    def receive_user_authentication_message(self, data):
        ...
        if auth_token is not None:  # auth with token given by yaapp and passed to the streamer by the client
            ...
            if username is not None and api_key is not None:  # auth with username and api_key (for old clients compatibility)
                ...
                user = yaquery(query_manager.QUERY_TYPE_USER_BY_USERNAME, username)
                user_id = None
